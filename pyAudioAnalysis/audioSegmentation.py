...
def labels_to_segments(labels, class_probabilities, window):
    """
    ARGUMENTS:
    ...
    while index < len(labels) - 1:
        ...
        while True:
            cur_prob += class_probabilities[index]
            seg_len += 1
            index += 1
            ...
            if (compare_flag != cur_label) | (index == len(labels) - 1):
                ...
                seg_probs.append(float(cur_prob/seg_len))
                break
    ...
    return segments, classes, seg_probs
...
def plot_segmentation_results(flags_ind, flags_ind_gt, class_names, mt_step,
    ...
    if min_len > 0:
        print("len(flags_ind): ", len(flags_ind))
        print("len(flags_ind_gt): ", flags_ind_gt)
        print("flags_ind: ", len(flags_ind))
        print("flags_ind_gt: ", len(flags_ind_gt))
        print("flags_ind[0:min_len]: ", flags_ind[0:min_len])
        print("flags_ind_gt[0:min_len]: ", flags_ind_gt[0:min_len])
        x = flags_ind[0:min_len] == flags_ind_gt[0:min_len]
        print("X: ", x)
        print("len(x): ", len(x))
        print("float(min_len): ", float(min_len))
...
def mid_term_file_classification(input_file, model_name, model_type,
    ...
    for col_index in range(mt_feats.shape[1]):
        ...
        # classify vector:
        label_predicted, prob = at.classifier_wrapper(classifier, model_type, feature_vector)
...
def load_ground_truth(gt_file, labels, class_names, mid_step, plot_results):
    ...
    if os.path.isfile(gt_file):
        ...
        # print("labels_new: ", len(labels_new))
